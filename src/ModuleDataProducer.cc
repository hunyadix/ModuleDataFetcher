...
int ModuleDataProducer::convertPhaseZeroOfflineLadderToOnlineLadderCoordinate(const int& layer, const int& ladder)
{
	...
	switch(layer)
	{
		// Ladder coordinate ranges: [0, 20], [0, 32], [0,44]
		// Quarter sizes: 5, 8, 11
		// Rule: First quarter, two quarters, one quarter
		// The second upper range should be three times the first
		// The  third upper range should be  four times the first
		case 1:
